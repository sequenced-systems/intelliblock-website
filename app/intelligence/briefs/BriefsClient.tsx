...
export default function BriefsClient() {
  const [data, setData] = useState<BriefsPayload>(EMPTY)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/data/briefs.json')
      .then(r => r.json())
      .then((payload: BriefsPayload) => setData(payload))
    ...
    <main>
      ...
      <section className="section section--white" aria-labelledby="briefs-feed-headline">
        <div className="container">
          ...
          {loading ? (
            <div className="briefs-empty" role="status" aria-live="polite">
              Loading scored briefs from the HEARTBEAT pipeline…
            </div>
          ) : count === 0 ? (
